...
input_dir = f"{PROJ_DIR}/data/processed_data/tokenized_claims/{SECTION}-{YEAR_THRESHOLD}"

# Paths to the pre-year and post-yearP0+r\P0+r\ claims files
pre_year_claims_file = os.path.join(input_dir, f'pre_{YEAR_THRESHOLD}_claims_tokenized.csv')
post_year_claims_file = os.path.join(input_dir, f'post_{YEAR_THRESHOLD}_claims_tokenized.csv')

# Output directory for predictions
output_dir = f"{PROJ_DIR}/predictions/claims-model/"
os.makedirs(output_dir, exist_ok=True)

# Model path
model_path = f"{PROJ_DIR}/src/model_training/claims/model-checkpoint/last.ckpt"
...
class BertClassifier(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        outputs = self(
            ...
            attention_mask=batch['attentioP0+r\P0+r\P0+r\n_mask']
        )
...
def predict_on_dataset(claims_file, output_file):
    print(f"Processing dataset: {claims_file}")
    ...
    print(f"Total evaluation time for {claims_file}: {total_time:.2f} seconds")
    print(f"Done predicting on {claims_file}")

# Predict on pre-year claims
pre_output_file = os.path.join(output_dir, f'pre_{YEAR_THRESHOLD}_claims_predictions.csv')
predict_on_dataset(pre_year_claims_file, pre_output_file)

# Predict on post-year claims
post_output_file = os.path.join(output_dir, f'post_{YEAR_THRESHOLD}_claims_predictions.csv')
predict_on_dataset(post_year_claims_file, post_output_file)
