...
# Load the config file to get the section and year
...
input_dir = f"{PROJ_DIR}/data/processed_data/details/{SECTION}-{YEAR_THRESHOLD}"

# Paths to the pre-year and post-year details files
pre_year_details_file = os.path.join(input_dir, f'pre_{YEAR_THRESHOLD}_details_text.csv')
post_year_details_file = os.path.join(input_dir, f'post_{YEAR_THRESHOLD}_details_text.csv')

# Output directory for tokenized details
output_dir = f"{PROJ_DIR}/data/processed_data/tokenized_details/{SECTION}-{YEAR_THRESHOLD}"
...
def chunk_and_tokenize_df(df, overlap):
    ...
    for index, row in df.iterrows():
        ...
        original_text = row['description_text']
        
        try:
            # Try to parse JSON, otherwise treat as string
            original_text = json.loads(original_text)
            original_text = ' '.join(original_text)  # Join JSON array of strings
        except (json.JSONDecodeError, TypeError):
            pass  # Continue if it's already a simple string
...
def process_and_tokenize_details_file(input_file, output_file):
    # Read in the DataFrame
    print(f"Reading data from {input_file}")
    df = pd.read_csv(input_file, dtype={'pgpub_id': str})
    print(f"Data shape: {df.shape}")

    # Check for and drop rows with missing 'description_text'
    df.dropna(subset=['description_text'], inplace=True)
    print(f"Data shape after dropping rows with NaN 'description_text': {df.shape}")
...
# Process pre-year details file
pre_output_file = os.path.join(output_dir, f'pre_{YEAR_THRESHOLD}_details_tokenized.csv')
process_and_tokenize_details_file(pre_year_details_file, pre_output_file)

# Process post-year details file
post_output_file = os.path.join(output_dir, f'post_{YEAR_THRESHOLD}_details_tokenized.csv')
process_and_tokenize_details_file(post_year_details_file, post_output_file)
